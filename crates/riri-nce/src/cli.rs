//! CLI logic for `nce` — exposed both via the `riri-nce` binary and the
//! `riri-napi-nce` JS shim.
...
use crate::{
    CheckEnginesInput, CheckEnginesOutput, LifecycleConfig, LifecycleOutput, NpmBumpResult,
    apply_engines_to_lockfile, apply_engines_update, check_engines_with_lifecycle,
};
...
fn check_data_freshness(args: &Args, data: &LifecycleData, today: NaiveDate) -> Option<i32> {
...
fn npm_bump_json(bump: Option<&NpmBumpResult>) -> serde_json::Value {
...
fn exit_code_for(output: &CheckEnginesOutput, lifecycle: &LifecycleOutput) -> i32 {
    if lifecycle.unsatisfiable {
        EXIT_UNSATISFIABLE
